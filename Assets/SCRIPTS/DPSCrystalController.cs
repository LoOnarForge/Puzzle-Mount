...
public class DPSCrystalController : MonoBehaviour
{
    ...
    private static readonly string[] LightIntensityFieldLabels =
    {
        "Red Light intensity",
        "Green Light intensity",
        "Yellow Light intensity",
        "Dark Blue Light intensity",
        "Purple Light intensity",
        "Blue Ice Light intensity",
    };
    ...
    // Inspector label for per-color default light intensity fields.
    public static string GetLightIntensityFieldLabel(int setIndex)
    {
        if (setIndex < 0 || setIndex >= LightIntensityFieldLabels.Length)
            return $"Crystal Type {setIndex + 1} Light intensity";

        return LightIntensityFieldLabels[setIndex];
    }
